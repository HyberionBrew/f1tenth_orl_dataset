...
def get_dict_from_tf(logdir, tag="train/pred returns"):
    ...
    print(logdir)
    # Use EventAccumulator to load the TensorBoard logs
    ea = event_accumulator.EventAccumulator(logdir)
    ea.Reload()  # Load all data from the log file

    # Check if the tag exists in the scalars
    if tag in ea.scalars.Keys():
        scalar_events = ea.scalars.Items(tag)
        for event in scalar_events:
            returns.append(event.value)
    else:
        print(f"Tag '{tag}' not found in the log file.")

    return returns

def get_dict_from_tf_deprecated(logdir, tag="train/pred returns"):
    returns = []
    #print("-----")
    #print(logdir)
    # discover the file in the folder
            ...
            for agent in os.listdir(os.path.join(logdir, reward, length)):
                ...
                for run in os.listdir(os.path.join(logdir, reward, length, agent)):
                    ...
                    if not os.path.isdir(folder_path):
                        continue
                    data = get_dict_from_tf(folder_path, tag=tag_pred)
                    data_std = get_dict_from_tf(folder_path, tag=tag_std)
                    # print(len(data))
...
def post_process_rewards(rewards_dict):
    ...
    for method in rewards_dict.keys():
        ...
        if method=="ground_truth" or method=="sim":
            continue
...
def plot_bars_from_dict(reward_dict, target, length, methods, sub_keys=["progress_weight"], path=None, add_title=""):
    print(methods)
    ...
    for i, method in enumerate(methods):
        ...
        for j, key_sub in enumerate(keys_sub):
            ...
            if i == 0:
                ax.bar(x_coord, mean, yerr=std, width=bar_width, label=key_sub, alpha=0.8, capsize=7, color=colors[j])
            else:
